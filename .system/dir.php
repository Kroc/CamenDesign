...
// dir.php : gives a directory listing for a folder
include "shared.php";
...
//==============================================================================
$path = (
    preg_match ('/^(?:(\.(?!\.)|[^.])+\/)*$/', @$_GET['path'])
    ? $_GET['path'] : ''
) or errorPage (
    'malformed_request.rem',
    'Error: Malformed Request',
    ['URL' => '?path=path/to/use/']
);
...
foreach ($dir as &$dir_item) {
    ...
    switch (pathinfo ($dir_item, PATHINFO_EXTENSION)) {
        ...
        case '':
            $icon = $dir_item == '..'
            ? 'design/icons/parent.png' : 'design/icons/folder.png';
            break;
        ...
        case 'png':
            ...
            $icon = ".cache/$path"
                    .pathinfo ($dir_item, PATHINFO_FILENAME).'!ICON.png'
            ;
            if (!file_exists (APP_ROOT.$icon)) {
                ...
                imagefill (
                    $buffer, 0, 0,
                    imagecolorallocatealpha ($buffer, 0, 0, 0, 127)
                );
                ...
                switch ($info[2]) {
                    case IMAGETYPE_JPEG : $image = imagecreatefromjpeg (
                        APP_ROOT."$path/$dir_item") or exit; break;
                    case IMAGETYPE_PNG  : $image = imagecreatefrompng  (
                        APP_ROOT."$path/$dir_item") or exit; break;
                }
                ...
                */
                imagefilledrectangle (
                    ...
                    (int) (((128-$width)  / 2) - 10),
                    (int) (((128-$height) / 2) - 10),
                    (int) (((128-$width)  / 2) + $width  + 10),
                    (int) (((128-$height) / 2) + $height + 10),
                    0xF8F8F8
                );
    ...
    @$html .= template_tags (template_load ('dir.item.html'), [
        'NAME' => $dir_item,
        'ICON' => $icon,
...
exit (templatePage (template_tags (template_load ('dir.html'), [
    'TITLE'	=> $path,
    'DIR'	=> $html
